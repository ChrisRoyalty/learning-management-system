...
export type Student = {
  ...
  email: string;
  courses: string[];
  batch: string; // NEW: student's batch/cohort
  joinDate: string; // YYYY-MM-DD
  status: StudentStatus;
};

const initialStudents: Student[] = [
  { id: "1", name: "Alice Cooper", email: "alice@example.com", courses: ["React Fundamentals", "UI/UX Design"], batch: "Batch A", joinDate: "2024-01-15", status: "Active" },
  { id: "2", name: "Bob Wilson", email: "bob@example.com", courses: ["Node.js Backend"], batch: "Batch B", joinDate: "2024-02-20", status: "Active" },
  { id: "3", name: "Charlie Brown", email: "charlie@example.com", courses: ["Python Data Science"], batch: "Batch A", joinDate: "2024-03-10", status: "Inactive" },
  { id: "4", name: "Diana Prince", email: "diana@example.com", courses: ["React Fundamentals", "Node.js Backend"], batch: "Batch C", joinDate: "2024-01-08", status: "Active" },
];
...
export default function StudentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter((s) =>
      [s.name, s.email, s.batch, ...s.courses].some((v) => v.toLowerCase().includes(term))
    );
  }, [q, students]);
            ...
            <TableBody>
              {filtered.map((s) => (
                <TableRow key={s.id}>
                  ...
                  <TableCell>{s.batch || <span className="text-zinc-400">—</span>}</TableCell>
              ...
              {filtered.length === 0 && (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-10 text-sm text-zinc-500">No students match your search.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
...
function StudentForm({ initial, onSave }: { initial?: Partial<Student>; onSave: (data: Partial<Student>) => void; }) {
  ...
  const [batch, setBatch] = useState(initial?.batch ?? ""); // NEW state
  ...
  return (
    <DialogContent className="sm:max-w-md">
      ...
      <div className="grid gap-3 py-2">
        ...
        <div className="grid gap-1.5">
          <Label htmlFor="batch">Batch / Cohort</Label>
          <Input id="batch" value={batch} onChange={(e) => setBatch(e.target.value)} placeholder="e.g. Batch A, 2025 Cohort 1" />
        </div>
        <div className="grid gap-1.5">
      ...
      <DialogFooter>
        ...
        <DialogClose asChild>
          <Button onClick={() => onSave({ name, email, courses: courses.split(",").map(s => s.trim()).filter(Boolean), batch, joinDate, status })}>Save</Button>
        </DialogClose>
      </DialogFooter>
    </DialogContent>
  );
